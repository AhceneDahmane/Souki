    ...
    <div className="flex min-h-[80vh] items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="mt-6">
          ...
          >
            <svg className={`w-3 h-3 transition-transform ${showCreds ? "rotate-90" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
            Comptes de test
          </button>
          {showCreds && (
            <div className="mt-3 space-y-2">
              {testAccounts.map((acc) => (
                <div key={acc.role} className="flex items-center justify-between px-3 py-2 rounded-lg bg-[#18181b] border border-[#27272a]">
                  <div>
                    <p className="text-xs font-medium text-white">{acc.role}</p>
                    <p className="text-[11px] text-zinc-500">{acc.email}</p>
                  </div>
                  <span className="text-[10px] text-zinc-600 font-mono">password123</span>
                </div>
              ))}
            </div>
          )}
